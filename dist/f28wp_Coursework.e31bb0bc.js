...
function Player(name, x, y, r, id) {
  ...
  this.defaultVel = 4;
  this.vel = this.defaultVel; //Set the mass of the player to PI*r^2

  this.mass = r * r * Math.PI; //Creates a variable to control the direction of the player (-vel is right and vel is left)

  this.dir = this.vel; //Create a variable to store the actual angle the player is facing (Using radians/degrees)
...
var Food = //Construcor takes in the x and y position of the food, as well as its radius
function Food(x, y, r) {
...
var foodRad = 12; //The mass of each food object (PI*r^2)

var foodMass = foodRad * foodRad * Math.PI; //Store the size of the level (from -worldSzie to worldSize in both the X and Y directions)

var worldSize = 7000; //Store the total ammount of food on the level

var totalFood = 4000; //The width and height of the game screen (1920 and 1080 are placeholder values)

var width = 1920;
var height = 1080; //A value used to check if the game is on its first frame

var first = true;
var playerName = 'helo'; //Used for positioning the camera (offsetting the x and y of the world)
...
var client; //The starting radius of each player

var playerStartRad = 32; //The mass the player must be greater than to be able to use the boost mechanic

var boostThreshold = 3000;
...
function startClient() {
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    $(".player,#" + players[i].id).append("<p>" + players[i].name + "</p>"); //Hide this element, using the unique ID

    $(".player,#" + i.toString()).hide();
  }

  console.log(playerName); //Set the width and height to the width and height of the window
...
function createPlayer() {
  ...
  $(".player,#" + client.id).append("<p>" + playerName + "</p>"); //Hide this element, using the unique ID

  $(".player,#" + client.id).hide(); //Add the client to the list of players, at the end of the list
...
function updateClient() {
  ...
  if (wait == waitTime) {
    //Start the client
    startClient();
    wait++; //Once this has happened it is no longer the first frame, dont run it again
  } else if (wait > waitTime) {
    if (!client.removed) {
      //Handle key input from the client
      keyInput(); //If the clients radius is not accurate (based off of its current mass)
      ...
      client.ang += client.dir * Math.PI / 180; //Update the x position using a sine graph and y position using a cosine graph (multiplying by velocity)

      client.x += client.vel * Math.sin(client.ang);
      client.y += client.vel * Math.cos(client.ang); //Set the screens offset to be equal to the clients position so that the correct part of the level is in the viewport
      ...
      if (client.vel > client.defaultVel) {
        ...
        client.vel *= 0.98; //Otherwise make sure the clients velocity is set to the default (not below, as sometimes the above code will make it lower than it should be)
...
function draw() {
  if (wait > waitTime) {
    ...
    offsetY = -client.y + height / 2; //Loop through all the food in the food array
      ...
      if (!food[i].removed) {
        if (food[i].x + offsetX >= 0 - foodRad && food[i].x + offsetX <= width + foodRad) {
          if (food[i].y + offsetY >= 0 - foodRad && food[i].y + offsetY <= height + foodRad) {
            ...
            collision(food[i]); //Move where its being drawn on the screen to correspond with its position in the viewport (Again using CSS's margin as the offset)
    ...
    for (var i = 0; i < players.length; i++) {
      ...
      $("p,.player,#" + i).html(players[i].name).css("color", "white").css("text-align", "center"); //Show the player if its in the game, otherwise remove it
  ...
  document.addEventListener('keydown', function (event) {
    if (!client.removed) {
      ...
      if (event.key === 'd') {
        ...
        client.dir = -client.defaultVel; //If a is pressed
      } else if (event.key === 'a') {
...
function loop(timestamp) {
  //Draw the game
  draw(); //Run the loop the next time the window is ready to update (Runs until the game is closed)

  window.requestAnimationFrame(loop);
} //Run the loop when the window is ready to update


window.requestAnimationFrame(loop); //RANDOM NUMBER GENERATOR
...
function collision(currentFood) {
  if (!client.removed) {
    ...
    var dist = Math.sqrt((currentFood.x - (client.x - 16)) * (currentFood.x - (client.x - 16)) + (currentFood.y - (client.y - 16)) * (currentFood.y - (client.y - 16))); //If the food is within the radius of the player

    if (dist < client.r + foodRad) {
      //Get the index of this piece of food in the array
      var i = food.indexOf(currentFood); //Set new x and y 

      socket.emit('eaten', i);
      food[i].removed = true; //Add the mass of this piece of food to the clients mass
...
socket.on('levelData', function (foo, pla) {
...
//# sourceMappingURL=/f28wp_Coursework.e31bb0bc.js.map